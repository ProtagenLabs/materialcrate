...
async function renderPdfThumbnail(pdfBuffer: Buffer): Promise<Buffer | null> {
...
export async function pdfToThumbnailBuffer(pdfBuffer: Buffer): Promise<Buffer | null> {
  try {
    const timeout = new Promise<null>((_, reject) =>
      setTimeout(() => reject(new Error("PDF render timeout")), PDF_RENDER_TIMEOUT_MS),
    );
    return await Promise.race([renderPdfThumbnail(pdfBuffer), timeout]);
  } catch {
    return null;
  }
}
